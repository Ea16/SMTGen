(set-logic QF_BV)
(set-option :produce-models true)
(declare-const bv_0 (_ BitVec 256))
(declare-const bv_1 (_ BitVec 256))
(declare-const bv_2 (_ BitVec 256))
(declare-const bv_3 (_ BitVec 256))
(declare-const bv_4 (_ BitVec 256))
(declare-const bv_5 (_ BitVec 256))
(declare-const bv_6 (_ BitVec 256))
(declare-const bv_7 (_ BitVec 256))
(declare-const bv_8 Bool)
(declare-const bv_9 Bool)
(declare-const bv_10 (_ BitVec 256))
(declare-const bv_11 (_ BitVec 256))
(declare-const bv_12 (_ BitVec 256))
(declare-const bv_13 (_ BitVec 256))
(declare-const bv_14 (_ BitVec 256))
(declare-const bv_15 (_ BitVec 256))
(declare-const bv_16 (_ BitVec 256))
(declare-const bv_17 (_ BitVec 256))
(declare-const bv_18 (_ BitVec 256))
(declare-const bv_19 (_ BitVec 256))
(declare-const bv_20 (_ BitVec 256))
(declare-const bv_21 (_ BitVec 256))
(declare-const bv_22 Bool)
(declare-const bv_23 (_ BitVec 256))
(declare-const bv_24 (_ BitVec 256))
(declare-const bv_25 (_ BitVec 256))
(declare-const bv_26 (_ BitVec 256))
(declare-const bv_27 (_ BitVec 256))
(declare-const bv_28 (_ BitVec 256))
(declare-const bv_29 (_ BitVec 256))
(declare-const bv_30 (_ BitVec 256))
(declare-const bv_31 (_ BitVec 256))
(declare-const bv_32 (_ BitVec 256))
(declare-const bv_33 Bool)
(declare-const bv_34 (_ BitVec 256))
(declare-const bv_35 (_ BitVec 256))
(declare-const bv_36 (_ BitVec 256))
(declare-const bv_37 (_ BitVec 256))
(declare-const bv_38 (_ BitVec 256))
(declare-const bv_39 (_ BitVec 256))
(declare-const bv_40 (_ BitVec 256))
(declare-const bv_41 (_ BitVec 256))
(declare-const bv_42 (_ BitVec 256))
(declare-const bv_43 (_ BitVec 256))
(declare-const bv_44 (_ BitVec 256))
(declare-const bv_45 (_ BitVec 256))
(declare-const bv_46 (_ BitVec 256))
(declare-const bv_47 (_ BitVec 256))
(declare-const bv_48 (_ BitVec 256))
(declare-const bv_49 (_ BitVec 256))
(declare-const bv_50 (_ BitVec 256))
(declare-const bv_51 (_ BitVec 256))
(declare-const bv_52 (_ BitVec 256))
(declare-const bv_53 (_ BitVec 256))
(declare-const bv_54 (_ BitVec 256))
(declare-const bv_55 (_ BitVec 256))
(declare-const bv_56 (_ BitVec 256))
(declare-const bv_57 (_ BitVec 256))
(declare-const bv_58 (_ BitVec 256))
(declare-const bv_59 (_ BitVec 256))
(declare-const bv_60 (_ BitVec 256))
(declare-const bv_61 (_ BitVec 256))
(declare-const bv_62 (_ BitVec 256))
(assert (= bv_2 (bvlshr bv_1 bv_0)))
(assert (= bv_3 (bvadd bv_0 bv_1)))
(assert (= bv_4 (bvor bv_1 bv_0)))
(assert (= bv_5 (bvnot bv_0)))
(assert (= bv_6 (bvurem bv_2 bv_5)))
(assert (= bv_7 (bvsub bv_4 bv_0)))
(assert (= bv_8 (bvsge bv_4 bv_3)))
(assert (= bv_9 (bvuge bv_5 bv_1)))
(assert (= bv_10 (bvsrem bv_4 bv_0)))
(assert (= bv_11 (bvshl bv_2 bv_1)))
(assert (= bv_12 (bvand bv_7 bv_2)))
(assert (= bv_13 (bvurem bv_11 bv_6)))
(assert (= bv_14 (bvneg bv_12)))
(assert (= bv_15 (bvxor bv_11 bv_7)))
(assert (= bv_16 (bvlshr bv_15 bv_1)))
(assert (= bv_17 (bvneg bv_0)))
(assert (= bv_18 (bvneg bv_16)))
(assert (= bv_19 (bvlshr bv_18 bv_10)))
(assert (= bv_20 (bvand bv_7 bv_18)))
(assert (= bv_21 (bvor bv_19 bv_6)))
(assert (= bv_22 (bvsle bv_0 bv_1)))
(assert (= bv_23 (bvmul bv_0 bv_21)))
(assert (= bv_24 (bvadd bv_5 bv_13)))
(assert (= bv_25 (bvadd bv_11 bv_7)))
(assert (= bv_26 (bvand bv_6 bv_4)))
(assert (= bv_27 (bvmul bv_10 bv_2)))
(assert (= bv_28 (bvsrem bv_3 bv_11)))
(assert (= bv_29 (bvnot bv_4)))
(assert (= bv_30 (bvshl bv_4 bv_25)))
(assert (= bv_31 (bvurem bv_26 bv_13)))
(assert (= bv_32 (bvlshr bv_1 bv_29)))
(assert (= bv_33 (bvuge bv_5 bv_14)))
(assert (= bv_34 (bvlshr bv_12 bv_14)))
(assert (= bv_35 (bvshl bv_2 bv_7)))
(assert (= bv_36 (bvand bv_18 bv_35)))
(assert (= bv_37 (bvsub bv_34 bv_20)))
(assert (= bv_38 (bvneg bv_1)))
(assert (= bv_39 (bvadd bv_7 bv_4)))
(assert (= bv_40 (bvsub bv_27 bv_15)))
(assert (= bv_41 (bvmul bv_32 bv_37)))
(assert (= bv_42 (bvxor bv_37 bv_28)))
(assert (= bv_43 (bvand bv_34 bv_32)))
(assert (= bv_44 (bvxor bv_43 bv_2)))
(assert (= bv_45 (bvand bv_24 bv_12)))
(assert (= bv_46 (bvxor bv_3 bv_40)))
(assert (= bv_47 (bvshl bv_44 bv_26)))
(assert (= bv_48 (bvnot bv_3)))
(assert (= bv_49 (bvand bv_48 bv_45)))
(assert (= bv_50 (bvand bv_11 bv_38)))
(assert (= bv_51 (bvsub bv_17 bv_36)))
(assert (= bv_52 (bvudiv bv_31 bv_14)))
(assert (= bv_53 (bvor bv_17 bv_41)))
(assert (= bv_54 (bvlshr bv_37 bv_38)))
(assert (= bv_55 (bvadd bv_26 bv_19)))
(assert (= bv_56 (bvshl bv_42 bv_53)))
(assert (= bv_57 (bvadd bv_4 bv_24)))
(assert (= bv_58 (bvudiv bv_1 bv_29)))
(assert (= bv_59 (bvor bv_32 bv_23)))
(assert (= bv_60 (bvshl bv_10 bv_0)))
(assert (= bv_61 (bvlshr bv_45 bv_20)))
(assert (= bv_62 (bvneg bv_4)))
(declare-const z_0 Bool)
(declare-const z_1 Bool)
(declare-const z_2 Bool)
(declare-const z_3 Bool)
(declare-const z_4 Bool)
(declare-const z_5 Bool)
(declare-const z_6 Bool)
(declare-const z_7 Bool)
(declare-const z_8 Bool)
(declare-const z_9 Bool)
(assert (= z_0 (= bv_62 bv_52)))
(assert (= z_1 (= bv_24 bv_11)))
(assert (= z_2 (= bv_18 bv_46)))
(assert (= z_3 (= bv_10 bv_7)))
(assert (= z_4 (= bv_20 bv_30)))
(assert (= z_5 (= bv_54 bv_60)))
(assert (= z_6 (= bv_57 bv_50)))
(assert (= z_7 (= bv_21 bv_14)))
(assert (= z_8 (= bv_44 bv_32)))
(assert (= z_9 (= bv_6 bv_38)))
(assert ( or z_2 ( and ( or z_9 z_3 ) ( and ( or z_4 ( or z_0 z_7 ) ) ( or ( or z_6 ( and z_8 z_1 ) ) z_5 ) ) ) ) )
(check-sat)
(get-model)
(exit)
